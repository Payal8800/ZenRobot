...
async def lego(event):
    ...
    try:
        ...
        await telethn.send_file(
            event.chat_id, file=fname, caption=f"ʟᴏɢᴏ ɢᴇɴᴇʀᴀᴛᴇᴅ ʙʏ ʀᴀᴅʜᴀ ✘ ʀᴏʙᴏᴛ\nRequested By:- [{event.sender.first_name}](tg://user?id={event.sender.id})\nCreated By: [『 ʀᴀᴅʜᴀ ✘ ʀᴏʙᴏᴛ 』](https://t.me/RadhaXRobot_Bot)"
        )
        await pesan.delete()
        if os.path.exists(fname):
            os.remove(fname)
    except Exception as e:
        await event.reply(f"ғʟᴏᴏᴅᴡᴀɪᴛ ᴇʀʀᴏʀ, ʀᴇᴩᴏʀᴛ ᴛʜɪs ᴀᴛ @RadhaX2Support")
